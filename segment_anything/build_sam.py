...
def build_sam_vit_h(args):
    return _build_sam(
        ...
        use_multimodal_prompt=getattr(args, 'use_multimodal_prompt', True),
...
def build_sam_vit_b(args):
    return _build_sam(
        ...
        clip_model_path=getattr(args, 'clip_model_path', None),
        num_classes=getattr(args, 'num_classes', 8),
        use_pnurl=getattr(args, 'use_pnurl', False),
        pnurl_config=getattr(args, 'pnurl_config', None),
        use_coop_prompt=getattr(args, 'use_coop_prompt', False),
        coop_config=getattr(args, 'coop_config', None),
    )
...
def _build_sam(
    ...
    sam = Sam(
        ...
        use_pnurl=use_pnurl,
        pnurl_config=pnurl_config,
        use_coop_prompt=use_coop_prompt,
        coop_config=coop_config,
    )
    # sam.train()
    if checkpoint is not None:
        with open(checkpoint, "rb") as f:
            # 训练保存的checkpoint通常包含optimizer等非tensor对象，直接使用weights_only=False
            # 这是可信的checkpoint（来自训练过程），所以可以安全加载
