...
# Optional CLIP import (will be None if clip isn't installed)
try:
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        # If using CLIP as the backbone, load the CLIP model and keep reference to its visual module.
        # We load CLIP lazily during init so that users without the package get a clear error only when requested.
        self.clip = None
        if self.use_clip:
            if _clip is None:
                raise ImportError(
                    "Requested use_clip=True but the 'clip' package is not installed. Install with `pip install git+https://github.com/openai/CLIP.git` or set use_clip=False.`"
                )
            # load CLIP model (may download weights if not cached)
            # load onto CPU here; caller may move the entire model to desired device later
            self.clip, _ = _clip.load(self.clip_model_name, device="cpu", jit=False)
            # Ensure the visual module exists and is a ViT-like model
            if not hasattr(self.clip, "visual"):
                raise RuntimeError("Loaded CLIP model does not have a 'visual' module compatible with ViT-style access.")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # If CLIP backbone is enabled, route through CLIP.visual to obtain patch tokens
        if self.use_clip:
            # Expect self.clip.visual to follow CLIP's ViT implementation
            visual = self.clip.visual
            # follow CLIP visual forward up to obtaining token embeddings per patch
            # conv1 -> (B, C, Hc, Wc)
            ...
            # prepend class token
            # class_embedding may be named 'class_embedding' in some CLIP variants
            ...
            # layer norm before transformer if present
            if hasattr(visual, "ln_pre"):
                x_seq = visual.ln_pre(x_seq)

            # transformer expects (N, B, C)
            ...
            # remove class token and reshape to (B, Hc, Wc, C)
            x_patches = x_tr[:, 1:, :].contiguous().view(B, Hc, Wc, -1)

            # continue with neck expecting (B, C, H, W)
