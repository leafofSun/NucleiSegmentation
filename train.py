...
from utils import (
    ...
    point_guidance_loss,
    get_logger,
    physical_semantic_consistency_loss,
    density_map_loss,
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="vision",
        choices=["vision", "semantic"],
        help="训练阶段：vision(先训纯视觉) 或 semantic(引入医学知识全面提升)",
    )
                    ...
                    with torch.no_grad():
                        ...
                        if gt_hv_map_batch is not None:
                            ...
                            if inst_batch is not None:
                                ...
                                inst_map_resized = F.interpolate(
                                    inst_map,
...
def validate_one_epoch(args, model, val_loader, epoch, writer, rank):
    ...
    for batch, batched_input in enumerate(pbar):
        ...
        # 修复点：validation 不再硬编码 prompt，而是读取 DataLoader 返回的文本
        dynamic_text = batched_input.get("text_prompt", ["Cell nuclei"] * len(images))
...
def main(args):
    ...
    try:
        ...
        model = TextSam(
            ...
            # OT 已从 train.py 主流程移除。
            # 这里固定 use_ot=False，避免 semantic 阶段误走 OT 分支。
            # sg_epsilon/sg_iters 仅保留为兼容 TextSam 构造函数的默认参数。
            sg_epsilon=0.05,
            sg_iters=3,
        ...
        if world_size > 1:
            # 当前结构仍可能存在按 phase 开关的模块。
            # 如果后续出现 DDP unused parameter 报错，再将 find_unused_parameters 改为 True。
            model = DDP(
                model,
        ...
        # ======================================================================
        # Two-stage training: gradient freezing and parameter routing
        # ======================================================================
        if args.phase == "vision":
            if rank == 0:
                logger.info("🔥 [Phase 1: 视觉筑基] 冻结文本语义(CoOp/PNuRL)，全力微调视觉解码器与ASR！")

            # 1. 冻结语义层
            if hasattr(raw_model, "prompt_learner"):
                for p in raw_model.prompt_learner.parameters():
                    p.requires_grad = False

            if hasattr(raw_model, "pnurl"):
                for p in raw_model.pnurl.parameters():
                    p.requires_grad = False
            ...
            adapter_params = [
                p for n, p in raw_model.image_encoder.named_parameters()
                if "Adapter" in n and p.requires_grad
            ]

            if adapter_params:
                params.append({"params": adapter_params, "lr": args.lr * 0.1})

        elif args.phase == "semantic":
            if rank == 0:
                logger.info("🚀 [Phase 2: 语义拔高] 视觉底盘已稳固，解冻医学知识库、PNuRL与CoOp，OT已关闭，开启端到端精调！")
        ...
        if len(params) == 0:
            raise RuntimeError(
                ...
                "Please check phase/use_asr/use_pnurl/use_coop settings."
            )
        ...
        if args.resume and os.path.exists(args.resume):
            if rank == 0:
                logger.info(f"🔄 渐进式微调：仅加载模型权重 from: {args.resume}")
            ...
            try:
                ...
                if rank == 0:
                    logger.info("✅ 预训练权重加载成功！新增模块保持随机初始化。")
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            loss, m_loss, h_loss, a_loss = train_one_epoch(
                args=args,
                model=model,
                optimizer=optimizer,
